                        ...
                        <div className="bg-white rounded-[24px] shadow-sm border border-zinc-50 p-6">
                            ...
                            {Object.keys(connections).length > 0 ? (
                                <div className="space-y-4">
                                    {Object.entries(connections).map(([group, users]) => (
                                        <div key={group}>
                                            <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-2">
                                                {group === 'TEACHERS' ? 'Öğretmenler' : group === 'STUDENTS' ? 'Öğrenciler' : group === 'SCHOOL' ? 'Okul' : group === 'HQ' ? 'Genel Merkez' : group}
                                            </p>
                                            <div className="space-y-2">
                                                {users.map((u: ConnectionUser) => (
                                                    <Link key={u.id} href={`/profile/${u.id}`} className="flex items-center gap-3 p-2 rounded-xl hover:bg-zinc-50 transition-colors">
                                                        <div className="w-8 h-8 rounded-full bg-red-50 flex items-center justify-center text-xs font-bold text-red-500 shrink-0">
                                                            {u.firstName?.charAt(0) || 'U'}
                                                        </div>
                                                        <div className="min-w-0">
                                                            <p className="text-sm font-semibold text-zinc-800 truncate">{u.firstName} {u.lastName}</p>
                                                            <p className="text-[10px] text-zinc-400">{u.role === 'TEACHER' ? 'Öğretmen' : u.role === 'STUDENT' ? 'Öğrenci' : u.role === 'PRINCIPAL' ? 'Okul İdaresi' : u.role === 'HQ' ? 'Genel Merkez' : u.role}</p>
                                                        </div>
                                                    </Link>
                                                ))}
                                            </div>
                                ...
                                <p className="text-sm text-zinc-400">Henüz bağlantı yok.</p>
                            )}
                        </div>
