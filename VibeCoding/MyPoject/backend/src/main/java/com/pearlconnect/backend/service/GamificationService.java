...
public class GamificationService {
    ...
    private void awardPoints(Long userId, int pointsToAdd) {
        ...
        // Check for new badges (both point-based and hour-based)
    ...
    private void checkAndAwardBadges(User user) {
        ...
        int userHours = user.getTotalHours() != null ? user.getTotalHours() : 0;

        for (Badge badge : allBadges) {
            boolean qualifies = false;

            // Check hour-based badges (Pearl badges)
            if (badge.getRequiredHours() != null && badge.getRequiredHours() > 0) {
                if (userHours >= badge.getRequiredHours()) {
                    qualifies = true;
                }
            }

            // Check point-based badges
            if (badge.getRequiredPoints() != null && badge.getRequiredPoints() > 0) {
                if (userPoints >= badge.getRequiredPoints()) {
                    qualifies = true;
                }
            }
    ...
    public GamificationProfileDto getGamificationProfile(Long userId) {
        ...
        int totalHours = user.getTotalHours() != null ? user.getTotalHours() : 0;
